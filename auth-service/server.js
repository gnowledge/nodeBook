...
async function initializeDatabase() {
    ...
    const dbPath = path.join(dataPath, 'auth', 'users.db');
    
    try {
        // Ensure the auth directory exists
        await fs.mkdir(path.join(dataPath, 'auth'), { recursive: true });
        ...
        server.log.info(`📁 Using data path: ${dataPath}`);
    } catch (error) {
    ...
    if (userCount.count === 0) {
        ...
        const dataPath = process.env.DATA_PATH || path.join(__dirname, 'data');
        const adminDataDir = path.join(dataPath, 'graphs', 'users', '1');
