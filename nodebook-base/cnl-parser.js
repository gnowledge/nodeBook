...
const ATTRIBUTE_REGEX = /^\s*has\s+([^:]+):\s*(?:\+\+(.+?)\+\+\s*)?([^;*[]+?)(?:\s*\*([^*]+)\*)?(?:\s*\[(.+?)\])?\s*;?/gm;
...
function processNodeHeading(heading) {
    ...
    // Try the explicit format first (with **adjective** markers and [Type])
    let match = heading.match(HEADING_REGEX);
    if (match) {
        [, , adjective, baseName, nodeType] = match;
        ...
        // Try simple format (any text with optional [Type])
    ...
    // Extract node type from [Type] if present
    if (nodeType) {
        nodeType = nodeType.trim();
    } else {
        // Check if there's a [Type] in the baseName
        const typeMatch = baseName.match(/\[(.+?)\]/);
        if (typeMatch) {
            nodeType = typeMatch[1].trim();
            // Remove the [Type] from baseName and displayName
            baseName = baseName.replace(/\[.+?\]/, '').trim();
            displayName = displayName.replace(/\[.+?\]/, '').trim();
        } else {
            nodeType = 'individual'; // Default type
        }
    }
    ...
    return { 
        ...
        payload: { 
            ...
            options: { 
                id, 
                role: nodeType, 
                parent_types: [], // We'll handle parent types separately if needed
