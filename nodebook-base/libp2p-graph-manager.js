...
logDebug('LibP2PGraphManager module loaded.');
...
class LibP2PGraphManager {
    ...
    async initialize(dataPath) {
        ...
        logDebug('LibP2PGraphManager initialized with user data segregation and collaboration support.');
    }
    ...
    // Create a new graph with libp2p
    async createGraph(userId, graphName, options = {}) {
        ...
        // Create libp2p graph instance
        const graph = await LibP2PGraph.create(storagePath, graphId);
    ...
    async getGraph(userId, graphId, GraphClass = LibP2PGraph) {
        if (!userId || !graphId) {
        ...
        try {
            // Create libp2p graph instance from existing data
            const graph = await LibP2PGraph.create(graphMetadata.storagePath, graphId);
    ...
    async cleanup() {
        ...
        logDebug('LibP2PGraphManager cleanup completed.');
    }
}

module.exports = LibP2PGraphManager;
